...
var ddFieldKnownUnmapped = map[string]map[string]string{ // model -> DD field -> reason
	"ddTestResourceData": {
		...
		"Notes":                "read-only server field as of DefectDojo 3.2 (*[]Note, absent from TestRequest and PatchedTestRequest)",
	...
	"engagementResourceData": {
		...
		"CicdBuildServer":         "writable but not yet exposed — references the new DefectDojo 3.2 CICDInfrastructure model (renamed from build_server, which referenced Tool_Configuration); candidate for follow-up (see Phase 2 report)",
		"Created":                 "server-managed timestamp",
		...
		"CicdOrchestrationEngine": "writable but not yet exposed — references the new DefectDojo 3.2 CICDInfrastructure model (renamed from orchestration_engine, which referenced Tool_Configuration); candidate for follow-up (see Phase 2 report)",
		...
		"CicdScmServer":           "writable but not yet exposed — references the new DefectDojo 3.2 CICDInfrastructure model (renamed from source_code_management_server, which referenced Tool_Configuration); candidate for follow-up (see Phase 2 report)",
